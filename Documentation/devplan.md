...
### Phase 13 — Localization ✅ *done 2026-07-23*
A database-backed localizer with named `%data%` placeholder substitution, a cookie-persisted language switch, an admin screen for correcting translations, and a full Greek seed. The interface is offered in English and Greek.

**Exit criteria met**, verified in a browser: the whole interface renders in Greek — nav, forms, buttons, validation messages, factor-table legend and all — with the choice sticking across pages; an administrator edits a translation and the change shows on the next request. 388 tests pass (220 unit, 168 integration).

Decisions taken during the work:
- **The key is the English source string, not a symbolic token.** `L["Start a topic"]`, never `L["topic.create.button"]`. A missing translation then falls back to readable English, so a half-translated page is still a usable page and a new string works in English the moment a developer writes it — no key registry to keep in sync. The cost is that editing an English string is a new key; that is the right trade for a source language that lives in the views.
- **Named placeholders, not positional.** The source spec asks for "text %data% text" precisely so a translator can move the value: `%count% new` may become `νέα: %count%` or be reordered past another hole. `PlaceholderFormatter` fills the holes *after* translation, and `string.Format`'s `{0}` could not express that reordering. A hole with no argument is left visible (`%count%`) rather than blanked — a loud sign of a caller's mistake.
- **The translation table is data, edited live.** Text lives in `LocalizedTexts (Key, Culture, Value)`, cached whole in memory (a page asks for dozens of strings; a query each would cost more than the rest of the page). The cache is swapped by replacing one reference, so a reader mid-request sees the whole old snapshot or the whole new one, never a half-built one. The admin screen writes a row and invalidates the cache; the seed is insert-only, so a translator's correction is never overwritten by the next deploy.
- **A binary collation on the key column.** The model's default collation is `utf8mb4_unicode_520_ci` — case- *and* accent-insensitive — under which `"closed"` and `"Closed"` compare equal and collide on the `(Key, Culture)` primary key. That silently failed the *entire* seed (the seeder logs and moves on, so Greek would simply be absent). The key column is pinned to `utf8mb4_bin` so every distinct source string is its own row. **A collation is part of a column's correctness, not a formatting detail.**
- **The HTML encoder is widened to the full Unicode range.** Razor's default `HtmlEncoder` escapes everything outside Basic Latin, so every Greek string shipped as `&#x398;…` entities — correct in a browser, but bloated and unreadable in the source, and it cost an afternoon masquerading as "the translation isn't working" (the page *was* Greek; the test was string-matching the entities). `TextEncoderSettings(UnicodeRanges.All)` makes non-Latin text emit as itself. The right default for a platform whose point is a second alphabet.
- **Content is not translated, only the interface.** Topic subjects, proposals, comments and messages stay in the language their author wrote them in; translating them would be rewriting someone's words. The manual says so plainly, so a Greek reader is not surprised by English content.
- **The custom localizer answers to both contracts.** It implements the app's own `IAppLocalizer` (indexer + named `Format`) that views use, and the framework's `IStringLocalizer` so `AddDataAnnotationsLocalization` routes validation messages through the same store. Validation messages were rewritten as self-contained English sentences (`"Use at least 12 characters."`) rather than the framework's `{0}`-interpolated templates, so each is a clean translatable key with no property-name substitution to translate around.
- **Switching language is a POST.** `CultureController.Set` writes the framework culture cookie and `LocalRedirect`s back; a GET would let another site flip a visitor's language, and an unchecked return URL would be an open redirect.
